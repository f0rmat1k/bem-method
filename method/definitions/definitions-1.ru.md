...
Методология БЭМ — это система методов, способов и правил, ориентированных на решение практических задач веб-разработки:

* создание архитектуры проекта;
* разделение кода на логические части;
* повторное использование кодовой базы;
* создание одной предметной области.
...
* одиночными, например, `логотип` или `кнопка`;
...
Блоки можно перемещать по странице, между разными страницами или разными проектами.
...
Элемент — это часть реализации блока, которая не может существовать и использоваться в отрыве от этого блока.
...
Модификатор — это свойство блока или элемента, определяющее их внешний вид, состояние и поведение.
...
>[Соглашение по именованию БЭМ](../naming/naming.ru.md) связывает БЭМ-сущности с DOM-узлами с помощью CSS-классов.
...
В HTML микс блока и элемента будет выглядеть следующим образом:
...
При сборке такое распределение позволяет получить отдельную реализацию радиопереключателя для каждой платформы, при этом базовая функциональность блоков останется общей.
